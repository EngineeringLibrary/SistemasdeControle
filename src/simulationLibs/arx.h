...
Nas representações a seguir definiremos  \f$ A(z^{-1}) \f$, \f$ B(z^{-1}) \f$ conforme as equações 1 e 2 respectivamente.
    ...
    \f$ A(z^{-1})= 1 + a_1z^-1 + a_2z^{-2} +  ... + a_{n_a}z^{-n_a} \f$  Equação 1

    \f$ B(z^{-1})= 1 + b_1z^-1 + b_2z^{-2} +  ... + b_{n_b}z^{-n_b} \f$  Equação 2
...
Isto significa que os polinômios \f$ A(z^{-1}) \f$ e \f$ B(z^{-1}) \f$ representam sistemas discretos no tempo. Sendo eles polinômios arbitrários, o modelo ARX assume a forma da equação 3.
    ...
    \f$ y(k)=\frac{B(z^{-1})}{A(z^{-1})}u(k)+\frac{1}{A(z^{-1})}v(k) \f$
...
O modelo ARX (AutoRegressive with eXogenous inputs)apresentado no diagrama de blocos da figura 1,pode ser obtido a partir de um modelo polinomial linear mais geral de entradas e saídas resultando na equação 3 \citeAguirre2007 .

        \image <bmp> <Figuras/ARX.bmp> ‘‘Diagrama de Blocos do Modelo ARX. Fonte: \cite Isermann2011’’
...
Em que:
    ...
    \param \f$ nu \f$ Na classe é representado por \param nInputpar, ou seja, o número de atrasos nas entradas do sistema.
    \param \f$ ny \f$ Na classe é representado por \param nOutputpar, ou seja, o número de atrasos nas saídas do sistema.
    \param \f$ d \f$ Na classe é representado por \param delay, ou seja, o número de atrasos de transporte do sistema.
    \param \f$ qi \f$ Na classe é representado por \param qdtInputVar, ou seja, o número entradas aplicadas no sistema.
    \param \f$ qo \f$ Na classe é representado por \param qdtOutputVar, ou seja, o número de saídas do sistema.
